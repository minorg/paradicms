...
object GenericTestData {
  ...
  val objects: List[Object] =
    collections.flatMap(collection => store.getObjects(currentUserUri = currentUserUri, limit = Int.MaxValue, offset = 0, query = ObjectsQuery(collectionUri = Some(collection.uri))).objectsWithContext)
      .map(objectWithContext => objectWithContext.object_)
      .sortBy((object_ => object_.uri.toString()))
  val object_ = store.getObjects(currentUserUri = currentUserUri, limit = 1, offset = 0, query = ObjectsQuery(collectionUri = Some(collection.uri))).objectsWithContext(0).object_
